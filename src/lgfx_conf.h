...
// SPI设定
      ...
      cfg.freq_write = DIJI_LCD_FREQ_WRITE; // 发送时的SPI时钟（最大80MHz，四舍五入为80MHz的整数）。
      cfg.freq_read  = DIJI_LCD_FREQ_READ;  // 接收时的SPI时钟
      cfg.spi_3wire  = DIJI_LCD_SPI_3WIRE; // 如果用MOSI引脚进行接收，则设置为true
      ...
      cfg.pin_sclk = DIJI_LCD_SCLK_PIN; // 设置SPI SCLK引脚编号
      cfg.pin_mosi = DIJI_LCD_MOSI_PIN; // 设置SPI的MOSI引脚编号
      cfg.pin_miso = DIJI_LCD_MISO_PIN; // 设置SPI的MISO针脚编号（-1 = 禁用）。
      cfg.pin_dc   = DIJI_LCD_DC_PIN;   // 设置SPI的D/C针脚编号（-1 = 禁用）。
    ...
    { // 配置显示面板控制设置。
      ...
      cfg.pin_cs           =    DIJI_LCD_CS_PIN;  // 连接CS的引脚编号（-1 = 禁用）。
      cfg.pin_rst          =    DIJI_LCD_RST_PIN; // 连接RST的引脚编号 (-1 = 禁用)
      cfg.pin_busy         =    -1;  // 连接BUSY的引脚编号 (-1 = 禁用)

        // * 下面的设置对每个面板都有一般的默认值，如果你对某个项目不确定，可以把它注释出来并试一试。

      cfg.panel_width      =   DIJI_LCD_PANEL_WIDTH;  // 实际可显示的宽度
      cfg.panel_height     =   DIJI_LCD_PANEL_HEIGHT; // 实际可显示的高度
      ...
      cfg.invert           = DIJI_LCD_INVERT;    // 设定 是否反色，有些屏幕需要设置这个值才能获取正确的颜色
      cfg.rgb_order        = DIJI_LCD_RGB_ORDER; // true 为 RGB false 为 BGR
